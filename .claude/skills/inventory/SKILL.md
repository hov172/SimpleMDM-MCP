...
3. Optional: "skip apps/profiles/users" → `--no-apps`/`--no-profiles`/`--no-users`; "with raw JSON" → `--raw`; "accept partial" → `--allow-partial`; format words → `--format csv|md|docx|all` (default all); "full detail" → `--report-detail full`; "roster"/"device list"/"hand to the department"/"one row per device" → `--report-style roster` (people-facing: by-group sections with users + assignment groups inline; pair with `devicegroup:` populations).
