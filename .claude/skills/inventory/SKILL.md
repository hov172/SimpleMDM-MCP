...
5. Read `<outDir>/summary.txt`; relay the headline (matched/selected/fleet counts, findings, PARTIAL warning if any). Exit 2 with partial data is expected behavior — surface it, suggest `--allow-partial` only if the user accepts incomplete evidence. A very common, benign cause of PARTIAL on `--all` runs is **unenrolled devices**: SimpleMDM still lists them but their per-device endpoints return `422 device is not enrolled`, so the section is marked failed. Retrying does not help (no MDM channel until re-enrollment); explain the cause and offer `--allow-partial`, or `--search 'status:enrolled'` to exclude them. See `docs/inventory.md` → "Completeness model".
