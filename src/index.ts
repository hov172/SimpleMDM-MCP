...
export const TOOLS: Tool[] = [
  ...
  { name: "generate_report",
    ...
    inputSchema: { type: "object", properties: {
      ...
      spec: { type: "object", description: "Dynamic mode: a declarative report spec {title, pageStyle (a3-landscape|a4-landscape|letter-portrait), footerTitle?, mdName?, dataAdapter, sections:[{heading, table:{columns:[{key,header}], from, csvName?, filter?}}]}. dataAdapter is one of devices|apps|profiles|users|logs|posture; each section's table.from selects rows from the adapter result (key \"rows\"). Optional table.filter is an array of {field, op, value?} conditions (ANDed) — op one of eq|ne|contains|icontains|gt|lt|gte|lte|exists|absent|in; field supports dot-paths (e.g. \"attributes.name\") — keeps only matching rows (e.g. stale devices, missing-FileVault). Mutually exclusive with `report`." },
