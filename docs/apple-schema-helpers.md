...
- Cache refresh script: `scripts/sync-apple-device-schemas.mjs`, which enumerates supported Apple profile/declaration YAML paths, parses them with `yaml`, and writes normalized JSON.
