...
If you don't want Docker, use the source option below.
...
2. **Vulnerability Check** — a per-release table (CVEs fixed, actively-exploited, devices on the release, unfixed-to-latest); the actual CVE IDs per release are in `cve-detail.csv` / `vulnerability-check.csv`. On a **scoped** run (any selector) this is trimmed to the OS major-version ladders the selected devices are on — the iOS/iPadOS table and unrelated macOS majors drop out, but the full upgrade ladder within a kept major stays so you still see what they're missing.
...
if you collect it into a custom attribute named `xprotect_version` (e.g. a periodic script on each Mac
that reads the local XProtect bundle version and writes it back via the SimpleMDM API). Until then, XProtect reports
...
| `--report-style <style>` | omit for the dossier default (audit style: rollups, findings, per-device facts; the CLI accepts only `roster`/`flat` explicitly) \| `roster` (people-facing: by-group summary, type/model breakdowns, then one row per device with local users and assignment groups inline — the classic "hand this list to the department" export) \| `flat` (one single table with `device_group` as a column — the spreadsheet view) |
...
| `--findings-exclude <types>` | drop comma-listed finding types from the report (noise control, e.g. `assigned-app-missing`); excluded counts are disclosed in `summary.txt` |
| `--raw` | also write redacted raw device JSON (off by default) |
| `--allow-partial` | exit 0 even when transient per-device fetches (429/5xx/network) failed — those otherwise mark the export PARTIAL and exit 2. Deterministic `422 not enrolled` fetches are disclosed as known limitations and never mark PARTIAL |
| `--no-network-cache` | bypass the 24 h SOFA feed cache (also accepted by audit and logs runs) |
...
| `--out <dir>` | output directory (default `reports/inventory-YYYYMMDD-HHMMSS/` under the install root) |
...
findings degrade to status `unknown` instead of asserting, and — for **transient**
failures (429/5xx/network) — the run exits 2 unless you pass `--allow-partial`.
Deterministic `422 device is not enrolled` fetches are different: they are disclosed in
`summary.txt` as **known device limitations** but do not mark the export PARTIAL,
so permanently unenrolled stragglers can't downgrade every whole-fleet export forever. Secrets (FileVault recovery keys, firmware/Recovery Lock
