...
claude mcp add simplemdm \
  ...
  -- docker run --rm -i -e SIMPLEMDM_API_KEY -v "$PWD/reports:/app/reports" -v "$PWD/audit_log:/app/audit_log" simplemdm-mcp
```
...
(The `-v` mounts keep container-written files on your disk after the `--rm`
container exits: `reports/` for the report tools' output and `audit_log/` for the
write-safety audit trail — both folders are gitignored, and the mounts are
harmless no-ops on read-only deployments.)
...
**Optional — persist generated reports and the write audit trail.** The report
tools (`run_fleet_audit`, `run_device_logs_audit`, `run_inventory_report`) write
under `/app/reports`, and the write-safety gate logs to `/app/audit_log` — both
inside the container, which vanishes when the `--rm` container exits (the tool
response still includes the summary and report preview). Mount host directories
to keep the files:
```bash
docker run --rm -i --env-file .env -v "$PWD/reports:/app/reports" -v "$PWD/audit_log:/app/audit_log" simplemdm-mcp
```
...
claude mcp add simplemdm \
  -e SIMPLEMDM_API_KEY=your-api-key-here \
  -- docker run --rm -i -e SIMPLEMDM_API_KEY -v /absolute/path/to/SimpleMDM-MCP/reports:/app/reports -v /absolute/path/to/SimpleMDM-MCP/audit_log:/app/audit_log simplemdm-mcp
```
...
and mount `reports/` (report-tool output) and `audit_log/` (write-safety audit
trail) so container-written files survive the `--rm` container:
```bash
claude mcp add simplemdm \
...
Low and medium-tier tools execute directly (no token required). **Confirm mode can be disabled** by setting `SIMPLEMDM_CONFIRM_MODE=off`, which restores the prior behavior of immediate execution for high/critical tools — useful for headless/scriptable deployments. Disabling it logs a one-line stderr warning at startup so the weakened posture is visible. Read tools are always unaffected.
...
- `SIMPLEMDM_CONFIRM_MODE` (default `on` when `SIMPLEMDM_ALLOW_WRITES=true`) — set `off` to execute high/critical tools directly without a token. Read per-call (no restart needed to change it), and `off` logs a one-line stderr warning at startup.
...
- `MCP_WRITE_AUDIT_DIR` (default `audit_log/`, resolved against the install root) — directory for JSONL audit logs. **Docker:** the default resolves inside the `--rm` container, so mount it (`-v "$PWD/audit_log:/app/audit_log"`) to keep the audit trail across sessions.
...
- **`executive-summary` report** — a leadership-facing one-pager (fleet KPIs, risk summary, top recommendations) rendered by the same engine to md/csv/docx/pdf via `generate_report` with `report: "executive-summary"` or `dist/reports/cli.js executive-summary`. Its recommendations come from the **`recommend_fixes`** tool, which merges the certificate, DEP-token, compliance, and stale-device audits into one prioritized action list — each item naming the gated workflow prompt or manual step that fixes it — and auto-publishes to MunkiReport like the other audit tools. Known limitation: the report path cannot fetch APNs-certificate or DEP-token status, so those Risk Summary rows show "unknown" with an in-document caveat pointing to `get_certificate_expiration_audit` / `get_dep_token_audit` for authoritative status.
...
The server registers **203 tools** covering the full SimpleMDM API surface (including, within that total: 31 derived fleet-analytics tools, 16 MunkiReport tools, 16 Apple schema helper tools). Reads are always available; writes require `SIMPLEMDM_ALLOW_WRITES=true`. Every tool ships with MCP annotations (`readOnlyHint`, `destructiveHint`, `idempotentHint`) so compatible clients can render the correct confirmation UI.
